...
export async function isFfmpegAvailable(): Promise<boolean> {
  ...
  const { code } = await run("ffmpeg", ["-version"], 5000);
  ffmpegAvailable = code === 0;
  if (!ffmpegAvailable) {
    console.warn(
      "[video-frames] ffmpeg not found — reel heroes will fall back to Instagram's cover frame (which has a play glyph burned in). Install ffmpeg for better card images.",
    );
  }
...
export async function extractFrames(
  ...
  const dir = await mkdtemp(join(tmpdir(), "torque-reel-"));
